...
// {
//     // Check if an OTP already exists and is still valid (within 5 minutes)
//     $sql_check_otp = "SELECT otp, timestamp 
//                       FROM otp_table 
//                       WHERE phone_no = ?";
//     $stmt_check = $conn_hayleys_medicalapp->prepare($sql_check_otp);
//     $stmt_check->bind_param("s", $phone_no);
//     $stmt_check->execute();
//     $result_check = $stmt_check->get_result();

//     if ($result_check->num_rows > 0) {
//         // OTP exists and is still valid, resend the same OTP
//         $row = $result_check->fetch_assoc();
//         $otp = $row['otp'];
//         sendOtpMessage($phone_no, $otp);
//         echo json_encode([
//             "status" => "exists",
//             "message" => "OTP $otp sent to $phone_no. Please verify."
//         ]);
//     } else {
//         // Generate a new OTP as the old one is expired or doesn't exist
...
function sendOtpMessage($phone_no, $otp)
{
    ...
    $message = "Dear User,
We have received a request to reset your password for Fentons Medical System. Please use the following One-Time Password (OTP) to complete your request:
$otp
