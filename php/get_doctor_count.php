...
$sql_count = "SELECT COUNT(*) AS doctor_count FROM doctor WHERE CUSTOMERS_ID = ?";
...
if ($row = $result_count->fetch_assoc()) {
    echo json_encode(array("status" => "success", "doctor_count" => $row['doctor_count']));
} else {
    echo json_encode(array("status" => "error", "message" => "Failed to fetch doctor count"));
}
